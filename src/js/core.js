        ...
        chromeContentSettings.javascript.get(
          ...
          function(details) {
            ...
            url
              ...
              : (matchForbiddenOrigin = true);
            matchForbiddenOrigin
              ? updateIconAndMenu("inactive")
              : updateIconAndMenu(details.setting);
          }
        );
      }
    }
...
function checkVersion(callback) {
  ...
  chrome.storage.local.get(["version"], function(data) {
    var oldValue =
      data["version"] || window.localStorage.qjs_version || undefined;
...
function onUpdate() {
  ...
  old_rules = window.localStorage.qjs_rules
    ? JSON.parse(window.localStorage.qjs_rules)
    : null;
